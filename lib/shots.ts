...
export type Shot = {
   ...
   *
   * **後段是決定性的那一段,前段幾乎不影響結果。** 948 發按中位切成 2×2:
   *
   *              後段低    後段高
   *   前段低      8,260     9,009
   *   前段高      8,223     8,949
   *
   * 後段高低差 750,前段固定後段之後只差 37–60。早期資料看起來「前段越鬆越好」,
   * 其實是那幾場剛好前段鬆與後段高同時出現而已。
   *
   * 最乾淨的反證是兩場完全相反的動作:一場前段 22、9.5 圈、上升 124ms,另一場
   * 前段 96、9 圈、上升 93ms —— SP 中位 9,102 vs 9,025,p = 0.72,一模一樣。
   * 兩場唯一的共同點就是後段都高(74 與 85)。
   */
